...
```csv
store_id,store_name,address,x,y,time_window_start,time_window_end,product_id,demand
STORE_001,"Супермаркет Север","ул. Лесная, 10",10.0,20.0,09:00,12:00,PROD_001,10
STORE_001,"Супермаркет Север","ул. Лесная, 10",10.0,20.0,09:00,12:00,PROD_002,5
```

**Столбцы:**
- `store_id` — уникальный ID магазина
- `store_name` — витринное название магазина (используется в отчетах)
- `address` — строка с адресом/описанием точки доставки
- `x, y` — координаты магазина на карте
...
**Примечание:** один магазин может появляться в нескольких строках (разные товары). Название и адрес дублируются, чтобы сохранить единый файл потребностей.

### trucks.csv

```csv
truck_id,truck_name,vehicle_type,driver_name,capacity,cost_per_km,start_x,start_y
TRUCK_001,"Газель «Север 1»","ГАЗель NEXT","Иван Петров",50.0,10.5,0.0,0.0
```

**Столбцы:**
- `truck_id` — уникальный ID грузовика
- `truck_name` — отображаемое имя/позывной автомобиля
- `vehicle_type` — модель/тип ТС (используется для отчетов)
- `driver_name` — закреплённый водитель/экипаж
- `capacity` — грузоподъёмность (в условных тоннах)
...
```csv
route_id,truck_name,vehicle_type,driver_name,truck_capacity_t,store_name,address,time_window,product_name,quantity,weight_t,distance_to_store_km,arrival_time,departure_time,route_distance_km,route_cost_rub
"ROUTE_1","Газель «Север 1»","ГАЗель NEXT","Иван Петров",50.00,"Супермаркет Север","ул. Лесная, 10","09:00–12:00","Молоко",10,5.00,10.50,09:15:30,09:20:30,35.50,372.75
...
# ИТОГИ
# Суммарный пробег: 156.75 км
# Совокупная стоимость: 1846.34 ₽
# Позиций доставлено: 12
```

### schedule.xlsx (Excel)

Excel-файл «Расписание» содержит:
- русифицированные заголовки и подписи для транспорта, магазинов и товаров;
- стилизованные числовые форматы (км, ₽, количество, вес);
- автоматическую ширину колонок и отдельный блок «ИТОГИ» (пробег, стоимость, вес, количество доставок, число магазинов);
- данные, которые можно фильтровать/сортировать по маршруту, магазину, типу ТС и т. д.
